...
int main(int argc, char* argv[])
{
  ...
  TFile *ipf_check = new TFile(Form("%s%s_%04d_W%d.root", FILEPATH, CHECKFILENAME, run, EVENTTIMEWINDOWSWIDTH));

  auto opf = new TFile(TString::Format("%s%s_%04d_W%d.root", FILEPATH, STANDARDWAVEFILENAME, run, EVENTTIMEWINDOWSWIDTH).Data(), "RECREATE");
